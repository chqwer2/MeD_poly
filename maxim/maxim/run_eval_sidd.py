...
def main(_):
  ...
  Inoisy = np.float32(np.array(img['ValidationNoisyBlocksSrgb'])).reshape(-1, 256, 256, 3)
  Inoisy /= 255.

  filepath = os.path.join(input_dir, 'ValidationGtBlocksSrgb.mat')
  img = sio.loadmat(filepath)
  GT = np.float32(np.array(img['ValidationGtBlocksSrgb'])).reshape(-1, 256, 256, 3)
  GT /= 255.
  ...
  def _process_file(i):
    ...
    input_img = Inoisy[i] #np.asarray(Image.open(input_file).convert('RGB'),
                           # np.float32) / 255.
    if FLAGS.has_target:
      # target_file = target_filenames[i]
      target_img = GT[i]  #np.asarray(Image.open(target_file).convert('RGB'),
                              # np.float32) / 255.
